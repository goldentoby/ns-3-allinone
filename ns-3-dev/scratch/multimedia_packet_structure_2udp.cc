...
{ 
  ...
  NodeContainer c = NodeContainer (nSrc, nSrc2, gw0, gw1, nDst);
  ...
  NodeContainer nSrc2Gw0 = NodeContainer (nSrc2, gw0);
  ...
  NetDeviceContainer dSrc2Gw0 = accessLink.Install (nSrc2Gw0);
  ...
  Ipv4InterfaceContainer interfaces_src2;
  ...
  interfaces_src2 = address.Assign(dSrc2Gw0);
  ...
  Ptr<Socket> src2Socket = Socket::CreateSocket (nSrc2, TypeId::LookupByName ("ns3::UdpSocketFactory"));
  uint16_t src2port = 12345;
  Ipv4Address src2addr ("10.15.1.2");
  InetSocketAddress src2 = InetSocketAddress (src2addr, src2port);
  src2Socket->Bind (src2);
  ...
  for (float time = 0.0 ; time<stoptime; time+=0.03, idx++){
      if (v[idx] > udp_limit){
        ...
        do{
          ...
          Simulator::Schedule (Seconds (time), &SendStuff, src2Socket, dstaddr, dstport, udp_limit);
        ...
        Simulator::Schedule (Seconds (time), &SendStuff, src2Socket, dstaddr, dstport, current_size);

      }else {
        Simulator::Schedule (Seconds (time),&SendStuff, srcSocket, dstaddr, dstport, v[idx]);
        Simulator::Schedule (Seconds (time),&SendStuff, src2Socket, dstaddr, dstport, v[idx]);
      }
  }
