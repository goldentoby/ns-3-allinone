 ...
 */
 ...
 main (int argc, char *argv[])
 {
  ...
  //  uint32_t iframe = 50000;
  //  uint32_t pframe = 10000;
//    Simulator::Schedule (Seconds (0.1),&SendStuff, srcSocket, dstaddr, dstport, iframe);
    std::ifstream in;
    ...
    for (float time = 0.0 ; time<5.0; time+=0.03, idx++){
        if (v[idx] > 50000){
          ...
          Simulator::Schedule (Seconds (time), &SendStuff, srcSocket, dstaddr, dstport, v[idx]/2);
        }else Simulator::Schedule (Seconds (time),&SendStuff, srcSocket, dstaddr, dstport, v[idx]);
    }

    // int idx =0;
    // for (float time = 0.0 ; time<3.0; time+=0.03, idx++){
    //     if (idx % 30 == 0){
    //       Simulator::Schedule (Seconds (time),&SendStuff, srcSocket, dstaddr, dstport, iframe);
    //       Simulator::Schedule (Seconds (time),&SendStuff, srcSocket, dstaddr, dstport, iframe);
    //     }else Simulator::Schedule (Seconds (time),&SendStuff, srcSocket, dstaddr, dstport, pframe);
    // }
...
//    Simulator::Schedule (Seconds (3.1),&SendStuff, srcSocket, dstaddr, dstport);
 ...
 dstSocketRecv (Ptr<Socket> socket)
 {
  ...
  //  SendStuff (socket, Ipv4Address ("10.1.1.1"), address.GetPort (), packet->GetSize ());
 }